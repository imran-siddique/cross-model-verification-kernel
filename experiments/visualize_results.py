...
class ASCIIChart:
    ...
    def grouped_bar_chart(
        ...
        max_val = max(
            v for group_data in data.values() 
    ...
    def table(
        ...
        for row in data:
            row_str = "|" + "|".join(
                f" {str(row.get(c, '')).ljust(widths[c])} "
                for c in columns
            ) + "|"
...
class SVGChart:
    ...
    def bar_chart(
        ...
        svg_parts = [
            ...
            f'<style>',
            f'  .title {{ font: bold 18px sans-serif; }}',
            f'  .label {{ font: 12px sans-serif; }}',
            f'  .value {{ font: bold 12px sans-serif; }}',
            f'  .axis {{ stroke: #333; stroke-width: 1; }}',
            f'</style>',
    ...
    def grouped_bar_chart(
        ...
        max_val = max(
            v for gd in data.values() for v in gd.values()
        ) * 1.1
        ...
        svg_parts = [
            f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {width} {height}">',
            f'<style>',
            f'  .title {{ font: bold 20px sans-serif; }}',
            f'  .label {{ font: 12px sans-serif; }}',
            f'  .value {{ font: 10px sans-serif; }}',
            f'  .legend {{ font: 12px sans-serif; }}',
            f'  .axis {{ stroke: #333; stroke-width: 1; }}',
            f'</style>',
            f'<rect width="{width}" height="{height}" fill="white"/>',
        ]
...
class ResultsVisualizer:
    ...
    def generate_demo_figures(self):
        ...
        table_data = [
            ...
            {"method": "Claude Alone", "pass_rate": 0.852, "delta": 0.011, "loops": 1.0, "time": 2.3},
            {"method": "CMVK (GPT→Gem)", "pass_rate": 0.924, "delta": 0.083, "loops": 1.8, "time": 4.2},
            {"method": "CMVK (o1→Gem)", "pass_rate": 0.931, "delta": 0.090, "loops": 1.5, "time": 8.1},
        ]
        ...
        ascii_table = ASCIIChart.table(
            table_data,
            ["method", "pass_rate", "delta", "loops", "time"],
            "Main Results"
        )
        print(ascii_table)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Visualize CMVK experiment results"
    )
    parser.add_argument(
        "--input", "-i",
