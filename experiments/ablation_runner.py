...
from src.core.kernel import CrossModelVerificationKernel, set_reproducibility_seed
from src.generator import Generator
from src.verifier import Verifier
from src.datasets.humaneval_loader import load_humaneval
...
class AblationRunner:
    ...
    async def run_single_config(
        ...
        for i, problem in enumerate(self.problems):
            ...
            try:
                ...
                problem_result = await self._run_problem(
                    problem, config, seed + i
                )
    ...
    async def run_ablation_set(
        ...
        self._log(f"Running ablation set with {len(config_names)} configs, {self.num_runs} runs each")
    ...
    def compute_aggregates(
        ...
        from src.tools.statistics import mean, std, confidence_interval
    ...
    def save_results(
        ...
        with open(summary_path, "w") as f:
            ...
            for name, agg in aggregates.items():
                ...
                f.write(f"| {agg['name']} | {pr_mean:.1%} ± {pr_std:.1%} | {loops:.2f} | {time_s:.2f} |\n")
    ...
    def generate_comparison_table(
        ...
        from src.tools.statistics import welch_t_test
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run ablation studies for CMVK"
    )
    parser.add_argument(
        "--set", "-s",
