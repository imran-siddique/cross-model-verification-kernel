...
def run_experiment(
    ...
    for i, problem in enumerate(problems):
        ...
        logger.info(f"  Result: {'✓' if result['success'] else '✗'} in {result['elapsed_seconds']:.1f}s, {result['loops']} loops")
        logger.info(f"  Running: {success_count}/{len(results)} ({100*success_count/len(results):.1f}%)")
    ...
    logger.info(f"Pass rate: {summary['pass_rate']*100:.1f}% ({summary['successful']}/{summary['total_problems']})")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run reproducible CMVK experiments"
    )
    parser.add_argument(
        "--dataset", "-d",
        type=str,
