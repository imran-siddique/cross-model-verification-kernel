...
from src.core.types import ExecutionTrace, NodeState
...
class TestLateralThinkingIntegration:
    ...
    def test_node_state_fail_count(self):
        ...
        node.history.append(ExecutionTrace(
            step_id=0,
            code_generated="code1",
            verifier_feedback="good",
            status="success"
        ))
        assert node.fail_count == 0
        ...
        node.history.append(ExecutionTrace(
            ...
            verifier_feedback="error",
            status="failed"
        ))
        node.history.append(ExecutionTrace(
            step_id=2,
            code_generated="code3",
            verifier_feedback="error",
            status="failed"
        ))
    ...
    def test_lateral_thinking_workflow(self):
        ...
        node = NodeState(
            input_query="Write a function to calculate fibonacci number",
            status="pending"
        )
