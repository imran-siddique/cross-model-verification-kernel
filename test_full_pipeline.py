...
def visualize_traces(trace_files: list):
    ...
    cmd = [
        sys.executable,
        "-m", "src.tools.visualizer",
        "--latest",
        "--speed", "0.1"  # Fast playback for testing
    ]
...
Examples:
  ...
  python test_full_pipeline.py --list-only
    ...
    parser.add_argument(
        '--full',
        action='store_true',
        help='Run with full HumanEval dataset (164 problems)'
    )
    
    parser.add_argument(
        '--list-only',
        action='store_true',
        help='Only list available traces, skip benchmark'
    )
    ...
    print("  2. Run: python experiments/blind_spot_benchmark.py --dataset experiments/datasets/humaneval_full.json")
